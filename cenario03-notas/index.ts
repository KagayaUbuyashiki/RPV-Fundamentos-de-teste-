...
function calcularMedia(alunoId: number): IResultadoMedia {
    const INVALID_RESULT: IResultadoMedia = {
        media: 0,
        bonus: 0,
        mediaFinal: 0,
        ehValido: false
    ...
    if (!aluno) return INVALID_RESULT;
    
    const { p1, p2, p3, p4 } = aluno.notas;
    const notas = [p1, p2, p3, p4];

    if (notas.some(n => n < 0 || n > 10)) return INVALID_RESULT;
    
    const mediaPonderada = (p1 * 1 + p2 * 1 + p3 * 2 + p4 * 2) / 6;

    let bonus = 0;
    if (aluno.presenca >= 75) bonus += 0.5;
    if (aluno.entregouTrabalhos) bonus += 1.0;

    const mediaFinal = Math.min(Number((mediaPonderada + bonus).toFixed(2)), 10);
...
function verificarAprovacao(alunoId: number): IResultadoAprovacao {
    ...
    const getSituacao = (media: number): IResultadoAprovacao['situacao'] => {
