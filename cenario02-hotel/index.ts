...
function calcularReserva(reserva: IReserva): IResultadoReserva {
    const INVALID_BOOKING: IResultadoReserva = {
        valorDiaria: 0,
        valorTotal: 0,
        desconto: 0,
        ehValida: false
    ...
    if (!quarto) return INVALID_BOOKING;
    if (noites < 1 || noites > 30) return INVALID_BOOKING;
    if (hospedes < 1 || hospedes > quarto.capacidade) return INVALID_BOOKING;
    if (mes < 1 || mes > 12) return INVALID_BOOKING;

    const subtotalNoites = quarto.precoNoite * noites;
    
    const isAltaTemporada = [12, 1, 2].includes(mes);
    const precoComTemporada = isAltaTemporada ? subtotalNoites * 1.3 : subtotalNoites;

    const desconto = noites >= 3 ? precoComTemporada * 0.1 : 0;
    const custoCafe = cafeDaManha ? 30 * noites : 0;

    const valorTotal = Number((precoComTemporada - desconto + custoCafe).toFixed(2));
