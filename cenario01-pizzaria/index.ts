...
function calcularPedido({ itens }: IPedido): IResultadoPedido {
    const FALLBACK_INVALIDO: IResultadoPedido = {
        subtotal: 0,
        desconto: 0,
        taxaEntrega: 0,
        valorTotal: 0,
    ...
    if (!itens || itens.length === 0) return FALLBACK_INVALIDO;

    const totalPizzas = itens.reduce((acc, item) => acc + item.quantidade, 0);
    if (totalPizzas === 0 || totalPizzas > 5) return FALLBACK_INVALIDO;

    let subtotal = 0;
    let pizzasGrandesCount = 0;

    for (const item of itens) {
        const pizza = cardapio.find(p => p.id === item.pizzaId);
        if (!pizza) return FALLBACK_INVALIDO;

        const precoUnitario = pizza.preco + (item.bordaRecheada ? 8 : 0);
        subtotal += precoUnitario * item.quantidade;

        if (pizza.tamanho === 'G' || pizza.tamanho === 'GG') {
            pizzasGrandesCount += item.quantidade;
        }
    }

    if (subtotal < 20) return FALLBACK_INVALIDO;

    const desconto = pizzasGrandesCount >= 2 ? subtotal * 0.1 : 0;
    const taxaEntrega = subtotal > 80 ? 0 : 7;
    const valorTotal = Number((subtotal - desconto + taxaEntrega).toFixed(2));
